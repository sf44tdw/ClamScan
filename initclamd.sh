...
cp -a ${FRESHCLAM_CONFIG_FILE_NAME}{,.`date +%Y%m%d%H%M%S`.bak}
...
sed -i 's/^#ExcludePath \^\/proc\//ExcludePath \^\/proc\//g' ${CLAMD_CONFIG_FILE_NAME}
sed -i 's/^#ExcludePath \^\/sys\//ExcludePath \^\/sys\//g' ${CLAMD_CONFIG_FILE_NAME}
